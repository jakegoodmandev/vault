      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-muted/50">
            ...
            <th className="text-left p-3 font-medium">Credentials</th>
        ...
        <tbody>
          {items.length === 0 ? (
            <tr>
              <td colSpan={5} className="p-3 text-center text-muted-foreground">No providers found.</td>
            </tr>
          ) : items.map((p) => (
            <tr key={p.id} className="border-t hover:bg-muted/50">
              <td className="p-3 font-medium">{p.last_name}, {p.first_name}</td>
              <td className="p-3 text-muted-foreground">{p.registration_number}</td>
              <td className="p-3 text-muted-foreground">{p.credentials ?? "-"}</td>
